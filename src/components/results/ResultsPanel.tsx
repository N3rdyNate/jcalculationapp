    ...
    <div className="space-y-4">
      <Card>
        <div className="grid grid-cols-2 gap-4">
          <div className="border-r border-slate-200 pr-4">
            <p className="text-xs uppercase text-slate-500 font-medium">Heating Load</p>
            <p className="text-2xl font-bold text-red-600 mt-1">
              {formatBtuh(result.heatingTotal)}
            </p>
            <p className="text-sm text-slate-500">{formatKW(result.heatingTotal)}</p>
          </div>
          <div>
            <p className="text-xs uppercase text-slate-500 font-medium">Cooling Load</p>
      ...
      <Card title="Top contributors">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <p className="text-xs uppercase text-slate-500 font-medium mb-2">Heating</p>
            <ul className="space-y-1">
              {result.topHeatingContributors.map((c) => (
                <li key={c.component} className="flex justify-between text-sm">
                  <span className="text-slate-700">{c.label}</span>
                  <span className="text-slate-900 font-medium">
                    {formatBtuh(c.btuh)} <span className="text-slate-500">({formatPercent(c.percent)})</span>
                  </span>
                </li>
              ))}
            </ul>
          ...
          <div>
            <p className="text-xs uppercase text-slate-500 font-medium mb-2">Cooling</p>
            <ul className="space-y-1">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left border-b border-slate-200">
                <th className="py-2 font-medium text-slate-600">Component</th>
                <th className="py-2 font-medium text-slate-600 text-right">Heating</th>
                <th className="py-2 font-medium text-slate-600 text-right">Cooling (sens.)</th>
                <th className="py-2 font-medium text-slate-600 text-right">Cooling (lat.)</th>
              </tr>
            </thead>
            <tbody>
              {result.components.map((c) => (
                <tr key={c.component} className="border-b border-slate-100">
                  <td className="py-2 text-slate-700">{c.label}</td>
      ...
      <Card title="Methodology" description="Simplified Manual J / ASHRAE">
        <dl className="text-xs text-slate-600 space-y-1">
          <div className="flex justify-between">
            <dt>Climate zone</dt>
            <dd className="font-medium text-slate-900">{result.meta.climateZoneLabel}</dd>
          </div>
          <div className="flex justify-between">
